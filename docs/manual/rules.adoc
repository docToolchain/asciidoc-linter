...
. There must be a space after the = characters
. The heading text should start with an uppercase letter
...
. Block images have titles
. Image attributes are properly formatted
...
=== LIST001: List After Paragraph

Detects lists that directly follow a paragraph line.

==== Description

AsciiDoc needs an empty line between a paragraph and a list, including description lists (`Term::`).
Without it, the list items become part of the paragraph text, and no list is rendered.

The rule reports the first list item of each affected list.
A list may start directly after a section title, or after a block title (`.Title`), a block attribute line (`[...]`) or a comment that opens the block.
Inside a paragraph, such lines are paragraph text and do not end it.

==== Examples

.Broken list (will be flagged)
[source,asciidoc]
----
Required tools:
...
* Nested list items and list continuations (`+`)
* Lists after a definition list term (`Term::`)
* Content inside tables and verbatim blocks
