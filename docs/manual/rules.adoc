...
Detects list items that directly follow a paragraph line.

==== Description

AsciiDoc needs an empty line between a paragraph and a list.
...
Lists that follow a section title, a block title (`.Title`), a block attribute line (`[...]`) or a comment are fine.
