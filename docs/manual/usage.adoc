...
# Fail only on errors, tolerate warnings
...
|--fail-level
|info
|Lowest severity that makes the exit code 1 (error, warning, info)
...
The exit code is 0 if no finding reaches the `--fail-level`, otherwise 1.
With the default `info`, any finding fails the run.
Use `--fail-level error` to let warnings and info findings pass in CI.
